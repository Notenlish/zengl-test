...
void main() {
    ...
    if (dis <= cloudRadius) {
        ...
        vec2 texture_uv = vec2(u + time/48.0, v);

		float noise_threshold = 0.165;
        float fbm1 = noise(texture_uv);
	    float fbm_val = noise(texture_uv*size+fbm1+vec2(time*time_speed, 0.0)) * 0.3;
        fbm_val = mod(fbm_val, 1.0);
		
        if (fbm_val > noise_threshold && normal.z > 0.2) {
...
//            fbm1 = fbm(texture_uv);
//            fbm_val = fbm(texture_uv*size+fbm1+vec2(time*time_speed, 0.0)) * 0.3;

            vec3 NotfragColor = texture(planetTexture, texture_uv).bgr * lightColor; //lightColor is a uniform vec3
            float ls = max(dot(normal, -normalize(lightDirection)), 0.04); // luminosity
//            ls -= fbm_val;
