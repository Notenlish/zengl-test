...
vec3 cloudFinal(float ls, vec2 texture_uv) {
    // ld.x = ls
    // ld.y = dithered_ls
    vec3 NotfragColor = texture(planetTexture, texture_uv).rgb * lightColor;
...
void main() {
    ...
    if (dis<=cloudRadius) {
        ...
        if (isCloud == 1.0) {
            ...
            final = vec4(planetFinal(ld.y, texture_uv).bgr, 1.0);
        } else {
            final = texture(Texture, fragCoord).rgba;
        }
        fragColor = final;
    } else {
        fragColor = texture(Texture, fragCoord).rgba;
    }
}
